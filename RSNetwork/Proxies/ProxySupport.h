...
/*!
	@function _RSNetworRSHostDoesNeedProxy
	...
	@param host A RSHostRef representing the host which is to be checked.
		Must be non-NULL.  If this If this reference is not a valid
		RSHostRef, the behavior is undefined.
	@param bypasses A RSArrayRef of RSStringRef's indicating host
		names which should be bypassed (not use the proxy).  The
		exception list in the SystemConfiguration proxy dictionary
		can be used directly.
	@param localBypass A RSBOOLRefindicating "local" hosts do not need the proxy.
...
//extern BOOL _RSNetworRSHostDoesNeedProxy(RSHostRef host, RSArrayRef bypasses, RSNumberRef localBypass);

/*!
	@function _RSNetworkDoesNeedProxy
	...
	@result If a match for the host is not found, TRUE is returned.  
		If a match is not found, FALSE is returned.
*/
extern BOOL _RSNetworkDoesNeedProxy(RSStringRef hostname, RSArrayRef bypasses, RSNumberRef localBypass);
