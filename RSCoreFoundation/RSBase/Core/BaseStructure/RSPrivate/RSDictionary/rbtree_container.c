...
rbtree_container_node *rbtree_container_search(rbtree_container *tree, RSTypeRef key)
{
	...
	while (node) 
	{
        rbtree_container_node *cur = ({
            const __typeof__(((rbtree_container_node *)0)->rb_node) * __mptr = (node);
            (rbtree_container_node *)((char *)__mptr - offsetof(rbtree_container_node, rb_node));
        });
        
        //rb_entry(node, rbtree_container_node, rb_node);
