...
RSHandle __RSLoadLibrary(RSStringRef dir, RSStringRef lib) {
    ...
    if (dir) {
        fullPath = RSRetain(RSFileManagerStandardizingPath(RSStringWithFormat(RSSTR("%r/%r"), dir, lib)));
    } else {
        fullPath = RSRetain(lib);
    }
...
RSHandle _RSLoadLibrary(RSStringRef library) {
    ...
    if ((handle = __RSLoadLibrary(nil, library))) {
        return handle;
    }
    if ((handle = __RSLoadLibrary(RSSTR("/System/Library/Frameworks"), library))) {
        return handle;
    }
    if ((handle = __RSLoadLibrary(RSSTR("/Library/Frameworks"), library))) {
        return handle;
    }
    if ((handle = __RSLoadLibrary(RSSTR("~/Library/Frameworks"), library))) {
        return handle;
