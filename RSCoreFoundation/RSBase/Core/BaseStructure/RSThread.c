...
static void __RSRepeatTaskThreadInitialize(void);
static void __RSThreadPoolInitialize(void);
...
static void *__RSThreadEntry(void *context) {
    ...
    for (; ;) {
        ...
        if (thread->_entry) {
            RSAutoreleaseBlock(^{
                if (__RSThreadEntryIsBlock(thread)) {
                    void (^perform)(void) = (void(^)(void))thread->_entry;
                    perform();
                } else {
...
RSExport RSThreadRef RSThreadCreateWithBlock(RSAllocatorRef allocator, void (^block)(void), BOOL suspend) {
    return __RSThreadCreateInstance(allocator, nil, YES, block, nil, suspend, nil);
}

RSExport RSThreadRef RSThreadSetBlock(RSThreadRef thread, void (^block)(void)) {
