...
static BOOL __RSBPLObjectHeaderV0InitWithObject(RSTypeRef obj, __RSBinaryPropertyListObjectHeader* headerV0)
{
    ...
    else if (objID == datatype)
    {
        ...
        headerV0->_stct = ((cnt = RSDataGetLength(obj)) < ___LRSBPLData) ? cnt : ((void)(((__RSBinaryPropertyListObjectHeaderV1*)headerV0)->_count = cnt), ___LRSBPLData);
        return YES;
    }
    else if (objID == stringtype)
    {
        headerV0->_type = ___RSBPLStringASCII;
        headerV0->_stct = ((cnt = RSStringGetLength(obj)) < ___LRSBPLStringASCII) ? cnt : ((void)(((__RSBinaryPropertyListObjectHeaderV1*)headerV0)->_count = cnt), ___LRSBPLStringASCII);
        return YES;
    }
    else if (objID == arraytype)
    {
        headerV0->_type = ___RSBPLArray;
        headerV0->_stct = ((cnt = RSArrayGetCount(obj)) < ___LRSBPLArray) ? cnt : ((void)(((__RSBinaryPropertyListObjectHeaderV1*)headerV0)->_count = cnt), ___LRSBPLArray);
        return YES;
    }
    else if (objID == settype)
    {
        headerV0->_type = ___RSBPLSet;
        headerV0->_stct = ((cnt = RSSetGetCount(obj)) < ___LRSBPLSet) ? cnt : ((void)(((__RSBinaryPropertyListObjectHeaderV1*)headerV0)->_count = cnt), ___LRSBPLSet);
        return YES;
    }
    else if (objID == dicttype)
    {
        headerV0->_type = ___RSBPLDictionary;
        headerV0->_stct = ((cnt = RSDictionaryGetCount(obj)) < ___LRSBPLDictionary) ? cnt : ((void)(((__RSBinaryPropertyListObjectHeaderV1*)headerV0)->_count = cnt), ___LRSBPLDictionary);
        return YES;
    }
...
static void ___appendString(__RSBinaryPropertyListWrittenBuffer* buffer, __RSBinaryPropertyListObject* v1)
{
    ...
    if (!buf)
    {
        buf = length > 512 ? ((void)(shouldReleaseBuf = YES), RSAllocatorAllocate(RSAllocatorSystemDefault, length)) : BUF;
