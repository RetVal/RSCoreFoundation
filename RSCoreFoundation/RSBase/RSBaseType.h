...
typedef pthread_mutex_t RSSpinLock;
#define RSSpinLockInit ((pthread_mutex_t)PTHREAD_MUTEX_INITIALIZER)

RSInline void RSSpinLockLock(RSSpinLock *lock) {
    pthread_mutex_lock(lock);
}

RSInline void RSSpinLockUnlock(RSSpinLock *lock) {
    pthread_mutex_unlock(lock);
}
    
RSInline BOOL RSSpinLockTry(RSSpinLock *lock) {
...
RSInline void RSSpinLockUnlock(volatile RSSpinLock_t *lock) {
    MemoryBarrier();
    *lock = 0;
}
