...
typedef struct raw_hash_entry raw_hash_entry;

/// The primary hashtable struct
typedef struct raw_hash_table {
    ...
    raw_hash_entry **array;

    /// A count of the number of hash collisions.
    unsigned int collisions;

    /// Any flags that have been set. (See the raw_ht_flags enum).
    int flags;
...
/// Hashtable initialization flags (passed to raw_ht_init)
typedef enum {
...
/// @brief Initializes the raw_hash_table struct.
...
/// @brief Destroys the raw_hash_table struct and frees all relevant memory.
/// @param table A pointer to the hash table.
...
void raw_ht_insert(raw_hash_table *table, void *key, size_t key_size, void *value, size_t value_size);
