...
RSPrivate RSStringRef RSBasicHashCopyDescription(RSConstBasicHashRef ht, BOOL detailed, RSStringRef prefix, RSStringRef entryPrefix, BOOL describeElements) {
    ...
    RSBasicHashApply(ht, ^(RSBasicHashBucket bkt) {
        ...
        if (!describeElements) {
            vDesc = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<%p>"), (void *)bkt.weak_value);
            if (ht->bits.keys_offset) {
                kDesc = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<%p>"), (void *)bkt.weak_key);
            }
        } else {
...
RSPrivate void RSBasicHashShow(RSConstBasicHashRef ht) {
    RSStringRef str = RSBasicHashCopyDescription(ht, YES, RSSTR(""), RSSTR("\t"), NO);
#if defined(DEBUG)
    RSShow(str);
#endif
    RSRelease(str);
...
RSPrivate RSStringRef __RSBasicHashCopyDescription(RSTypeRef obj) {
    RSBasicHashRef ht = (RSBasicHashRef)obj;
    RSStringRef desc = RSBasicHashCopyDescription(ht, NO, RSSTR(""), RSSTR("\t"), YES);
    RSStringRef result = RSStringCreateWithFormat(RSAllocatorSystemDefault, RSSTR("<RSBasicHash %p [%p]>%@"), obj, RSAllocatorSystemDefault, desc);
    RSRelease(desc);
    return result;
}
