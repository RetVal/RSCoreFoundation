...
static BOOL __plistCheckIfNumberString(const UniChar* numStr, int length)
{
...
RSInline BOOL __plistDateCheckRange(int date, int min, int max)
{
...
static RSDateRef __plistRestoreDate(RSAllocatorRef allocator, RSStringRef value)
{
    static const int dateLength = 20;
    ...
    if (dateLength == RSStringGetLength(value))
    {
            ...
            (ptr[5] == 'Z'))
        {
            ...
            if (NO == __plistCheckIfNumberString(payload, 4)) return nil;
            gd.year = atoi((const char*)payload);
            
            memset(payload, 0, sizeof(payload));
            
            RSStringGetCharacters(value, RSMakeRange(5, 2), payload);
            if (NO == __plistCheckIfNumberString(payload, 2)) return nil;
            gd.month = atoi((const char*)payload);
            if (__plistDateCheckRange(gd.month, 1, 12) == NO) return nil;
            
            RSStringGetCharacters(value, RSMakeRange(8, 2), payload);
            if (NO == __plistCheckIfNumberString(payload, 2)) return nil;
            gd.day = atoi((const char*)payload);
            ...
            if (__plistDateCheckRange(gd.day, 1, maxm) == NO) return nil;

            RSStringGetCharacters(value, RSMakeRange(11, 2), payload);
            if (NO == __plistCheckIfNumberString(payload, 2)) return nil;
            gd.hour = atoi((const char*)payload);
            if (__plistDateCheckRange(gd.day, 0, 59) == NO) return nil;
            
            RSStringGetCharacters(value, RSMakeRange(14, 2), payload);
            if (NO == __plistCheckIfNumberString(payload, 2)) return nil;
            gd.minute = atoi((const char*)payload);
            if (__plistDateCheckRange(gd.day, 0, 59) == NO) return nil;
            
            RSStringGetCharacters(value, RSMakeRange(17, 2), payload);
            if (NO == __plistCheckIfNumberString(payload, 2)) return nil;
            gd.second = atoi((const char*)payload);
            if (__plistDateCheckRange(gd.day, 0, 59) == NO) return nil;
#endif
...
static RSTypeRef __RSPropertyListCopyValue(RSAllocatorRef allocator, RSTypeRef value, int tagID)
{
    ...
    switch (tagID)
    {
        ...
        case DATA_IX:
            ...
            break;
        case DATE_IX:
            _v = __plistRestoreDate(allocator, value);
            break;
        case REAL_IX:
        case INTEGER_IX:
        case TRUE_IX:
