...
extern mach_port_t _dispatch_runloop_root_queue_get_port_4CF(void);
...
RSExport RSArrayRef __RSStopAllThreads(void) {
    RSMutableArrayRef suspended_list = __RSArrayCreateMutable0(RSAllocatorSystemDefault, 0, NULL);
...
//        SInt32 result = RSUserNotificationDisplayAlert(0.0, RSUserNotificationCautionAlertLevel, NULL, NULL, NULL, RSSTR("High Mach Port Usage"), RSSTR("This application is using a lot of Mach ports."), RSSTR("Default"), RSSTR("Altern"), RSSTR("Other b"), &responseFlags);
//        if (0 != result) {
//            RSLog(3, RSSTR("ERROR"));
//        } else {
...
static void __RSRunLoopModeClassDeallocate(RSTypeRef rs) {
    RSRunLoopModeRef rlm = (RSRunLoopModeRef)rs;
    if (NULL != rlm->_sources0) RSRelease(rlm->_sources0);
    if (NULL != rlm->_sources1) RSRelease(rlm->_sources1);
    if (NULL != rlm->_observers) RSRelease(rlm->_observers);
    if (NULL != rlm->_timers) RSRelease(rlm->_timers);
    if (NULL != rlm->_portToV1SourceMap) RSRelease(rlm->_portToV1SourceMap);
    RSRelease(rlm->_name);
...
static RSStringRef __RSRunLoopClassDescription(RSTypeRef rs) {
    ...
    result = RSStringCreateMutable(RSAllocatorSystemDefault, 0);
#if DEPLOYMENT_TARGET_WINDOWS
    RSStringAppendFormat(result, NULL, RSSTR("<RSRunLoop %p [%p]>{wakeup port = 0x%x, stopped = %s, ignoreWakeUps = %s, \ncurrent mode = %r,\n"), rs, RSGetAllocator(rs), rl->_wakeUpPort, __RSRunLoopIsStopped(rl) ? "true" : "false", __RSRunLoopIsIgnoringWakeUps(rl) ? "true" : "false", rl->_currentMode ? rl->_currentMode->_name : RSSTR("(none)"));
#else
    RSStringAppendStringWithFormat(result, NULL, RSSTR("<RSRunLoop %p [%p]>{wakeup port = 0x%x, stopped = %s, ignoreWakeUps = %s, \ncurrent mode = %r,\n"), rs, RSGetAllocator(rs), rl->_wakeUpPort, __RSRunLoopIsStopped(rl) ? "true" : "false", __RSRunLoopIsIgnoringWakeUps(rl) ? "true" : "false", rl->_currentMode ? rl->_currentMode->_name : RSSTR("(none)"));
#endif
    RSStringAppendStringWithFormat(result, NULL, RSSTR("common modes = %r,\ncommon mode items = %r,\nmodes = %r}\n"), rl->_commonModes, rl->_commonModeItems, rl->_modes);
    return result;
}
...
static BOOL __RSRunLoopModeIsEmpty(RSRunLoopRef rl, RSRunLoopModeRef rlm, RSRunLoopModeRef previousMode) {
...
#endif
    BOOL libdispatchQSafe = pthread_main_np() && ((HANDLE_DISPATCH_ON_BASE_INVOCATION_ONLY && NULL == previousMode) || (!HANDLE_DISPATCH_ON_BASE_INVOCATION_ONLY && 0 == _RSGetTSD(__RSTSDKeyIsInGCDMainQ)));
    ...
    if (NULL != rlm->_sources0 && 0 < RSSetGetCount(rlm->_sources0)) return false;
    if (NULL != rlm->_sources1 && 0 < RSSetGetCount(rlm->_sources1)) return false;
    if (NULL != rlm->_timers && 0 < RSArrayGetCount(rlm->_timers)) return false;
...
void _RSRunLoopSetWindowsMessageQueueMask(RSRunLoopRef rl, uint32_t mask, RSStringRef modeName) {
    ...
    __RSRunLoopLock(rl);
    RSRunLoopModeRef rlm = __RSRunLoopFindMode(rl, modeName, true);
...
static void __RSRunLoopTimersToFireRecursive(RSRunLoopRef rl, RSRunLoopModeRef rlm, struct _collectTimersContext *ctxt) {
    ...
    if (NULL != rlm->_timers && 0 < RSArrayGetCount(rlm->_timers)) {
...
static RSRunLoopSourceRef __RSRunLoopModeFindSourceForMachPort(RSRunLoopRef rl, RSRunLoopModeRef rlm, __RSPort port) {	/* DOES CALLOUT */
    CHECK_FOR_FORK();
    RSRunLoopSourceRef found = rlm->_portToV1SourceMap ? (RSRunLoopSourceRef)RSDictionaryGetValue(rlm->_portToV1SourceMap, (const void *)(uintptr_t)port) : NULL;
    return found;
}
...
static void __RSRunLoopClassDeallocate(RSTypeRef rs) {
    ...
    __RSRunLoopPopPerRunData(rl, NULL);
    __RSRunLoopUnlock(rl);
...
static RSRunLoopRef __RSRunLoopCreate(pthread_t t) {
    ...
    loop->_winthread = 0;
#endif
    rlm = __RSRunLoopFindMode(loop, RSRunLoopDefaultMode, true);
    if (NULL != rlm) __RSRunLoopModeUnlock(rlm);
    return loop;
}

static RSMutableDictionaryRef __RSRunLoops = NULL;
...
RSExport RSRunLoopRef _RSRunLoopGet0(pthread_t t) {
    ...
    if (!__RSRunLoops)
    {
        ...
        if (!OSAtomicCompareAndSwapPtrBarrier(NULL, dict, (void * volatile *)&__RSRunLoops)) {
            RSRelease(dict);
        }
        RSRelease(mainLoop);
        RSSpinLockLock(&loopsLock);
    ...
    {
        _RSSetTSD(__RSTSDKeyRunLoop, (void *)loop, NULL);
...
void _RSRunLoopSetCurrent(RSRunLoopRef rl) {
    ...
    if (rl != currentLoop) {
        ...
        _RSSetTSD(__RSTSDKeyRunLoop, NULL, NULL);
...
RSRunLoopRef RSRunLoopGetMain(void) {
    CHECK_FOR_FORK();
    static RSRunLoopRef __main = NULL; // no retain needed
...
void RSRunLoopAddCommonMode(RSRunLoopRef rl, RSStringRef modeName) {
    ...
    if (!RSSetContainsValue(rl->_commonModes, modeName)) {
        RSSetRef set = rl->_commonModeItems ? RSSetCreateCopy(RSAllocatorSystemDefault, rl->_commonModeItems) : NULL;
...
static void __RSRunLoopDoObservers(RSRunLoopRef rl, RSRunLoopModeRef rlm, RSRunLoopActivity activity) {	/* DOES CALLOUT */
    ...
    RSRunLoopObserverRef *collectedObservers = (cnt <= 1024) ? buffer : (RSRunLoopObserverRef *)malloc(cnt * sizeof(RSRunLoopObserverRef));
    RSIndex obs_cnt = 0;
    for (RSIndex idx = 0; idx < cnt; idx++) {
...
{	/* DOES CALLOUT */
    CHECK_FOR_FORK();
    RSTypeRef sources = NULL;
    BOOL sourceHandled = false;
    
    /* Fire the version 0 sources */
    if (NULL != rlm->_sources0 && 0 < RSSetGetCount(rlm->_sources0)) {
    ...
    if (NULL != sources)
    {
        ...
        else
        {
            ...
            RSArraySort((RSMutableArrayRef)sources, RSOrderedAscending, (__RSRunLoopSourceComparator), NULL);
            for (RSIndex idx = 0; idx < cnt; idx++)
            {
...
static BOOL __RSRunLoopDoSource1(RSRunLoopRef rl, RSRunLoopModeRef rlm, RSRunLoopSourceRef rls
    ...
    if (__RSRunLoopIsValid(rls))
    {
        ...
        if (_LogRSRunLoop) { RSLog(RSLogLevelDebug, RSSTR("%p (%s) __RSRunLoopDoSource1 rls %p is invalid"), RSRunLoopGetCurrent(), *_RSGetProgname(), rls); }
        __RSRunLoopSourceUnlock(rls);
    }
    RSRelease(rls);
    __RSRunLoopLock(rl);
...
static void __RSArmNextTimerInMode(RSRunLoopModeRef rlm) {
    RSRunLoopTimerRef nextTimer = NULL;
        ...
        if (oldFireTSR < rlt->_fireTSR) {
            ...
            if (rlt_rl) {
                ...
                for (RSIndex idx = 0; idx < cnt; idx++) {
                    ...
                    if (rlm) {
                        __RSRepositionTimerInMode(rlm, rlt, true);
                    }
                }
                __RSRunLoopTimerFireTSRUnlock();
                for (RSIndex idx = 0; idx < cnt; idx++) {
...
RSExport BOOL _RSRunLoopFinished(RSRunLoopRef rl, RSStringRef modeName) {
    ...
    if (NULL == rlm || __RSRunLoopModeIsEmpty(rl, rlm, NULL)) {
        result = true;
    }
...
#elif DEPLOYMENT_TARGET_WINDOWS
            if (__RSRunLoopWaitForMultipleObjects(NULL, &dispatchPort, 0, 0, &livePort, NULL)) {
                goto handle_msg;
            }
#endif
        }
...
//            objc_clear_stack(0);
        ...
        __RSRunLoopWaitForMultipleObjects(waitSet, NULL, poll ? 0 : TIMEOUT_INFINITY, rlm->_msgQMask, &livePort, &windowsMessageReceived);
#endif
    ...
    handle_msg:;
        ...
        if (windowsMessageReceived) {
            ...
            __RSRunLoopWaitForMultipleObjects(waitSet, NULL, 0, 0, &livePort, NULL);
        ...
        if (MACH_PORT_NULL == livePort) {
	        ...
	        __RSRunLoopModeUnlock(rlm);
	        __RSRunLoopUnlock(rl);
            _RSSetTSD(__RSTSDKeyIsInGCDMainQ, (void *)6, NULL);
 	        _dispatch_main_queue_callback_4CF(msg);
            _RSSetTSD(__RSTSDKeyIsInGCDMainQ, (void *)0, NULL);
	        __RSRunLoopLock(rl);
	        __RSRunLoopModeLock(rlm);
 	        sourceHandledThisLoop = true;
...
SInt32 RSRunLoopRunSpecific(RSRunLoopRef rl, RSStringRef modeName, RSTimeInterval seconds, BOOL returnAfterSourceHandled) {     /* DOES CALLOUT */
    ...
    if (NULL == currentMode || __RSRunLoopModeIsEmpty(rl, currentMode, rl->_currentMode)) {
        BOOL did = false;
...
RSAbsoluteTime RSRunLoopGetNextTimerFireDate(RSRunLoopRef rl, RSStringRef modeName) {
    ...
    RSRunLoopTimerRef timer = (rlm && rlm->_timers && 0 < RSArrayGetCount(rlm->_timers)) ? (RSRunLoopTimerRef)RSArrayObjectAtIndex(rlm->_timers, 0) : NULL;
...
RSExport void _RSRunLoopStopMode(RSRunLoopRef rl, RSStringRef modeName) {
    ...
    RSRunLoopModeRef rlm;
    __RSRunLoopLock(rl);
    rlm = __RSRunLoopFindMode(rl, modeName, true);
    if (NULL != rlm) {
        rlm->_stopped = true;
        __RSRunLoopModeUnlock(rlm);
    }
...
void RSRunLoopPerformBlock(RSRunLoopRef rl, RSTypeRef mode, void (^block)(void)) {
    ...
    if (RSStringGetTypeID() == RSGetTypeID(mode)) {
        ...
        __RSRunLoopLock(rl);
        // ensure mode exists
        RSRunLoopModeRef currentMode = __RSRunLoopFindMode(rl, (RSStringRef)mode, true);
        if (currentMode) __RSRunLoopModeUnlock(currentMode);
        __RSRunLoopUnlock(rl);
        ...
        // ensure modes exist
        for (RSIndex idx = 0; idx < cnt; idx++) {
            RSRunLoopModeRef currentMode = __RSRunLoopFindMode(rl, (RSStringRef)values[idx], true);
            if (currentMode) __RSRunLoopModeUnlock(currentMode);
        }
        __RSRunLoopUnlock(rl);
        free(values);
...
void RSRunLoopAddSource(RSRunLoopRef rl, RSRunLoopSourceRef rls, RSStringRef modeName) {	/* DOES CALLOUT */
    ...
    if (modeName == RSRunLoopCommonModes)
    {
        RSSetRef set = rl->_commonModes ? RSSetCreateCopy(RSAllocatorSystemDefault, rl->_commonModes) : NULL;
        if (NULL == rl->_commonModeItems) {
    ...
    } else
    {
        ...
        if (NULL != rlm && !RSSetContainsValue(rlm->_sources0, rls) && !RSSetContainsValue(rlm->_sources1, rls)) {
            if (0 == rls->_context.version0.version) {
...
void RSRunLoopRemoveSource(RSRunLoopRef rl, RSRunLoopSourceRef rls, RSStringRef modeName) {	/* DOES CALLOUT */
    ...
    __RSRunLoopLock(rl);
    if (modeName == RSRunLoopCommonModes) {
        if (NULL != rl->_commonModeItems && RSSetContainsValue(rl->_commonModeItems, rls)) {
            RSSetRef set = rl->_commonModes ? RSSetCreateCopy(RSAllocatorSystemDefault, rl->_commonModes) : NULL;
        ...
        RSRunLoopModeRef rlm = __RSRunLoopFindMode(rl, modeName, false);
        if (NULL != rlm && ((NULL != rlm->_sources0 && RSSetContainsValue(rlm->_sources0, rls)) || (NULL != rlm->_sources1 && RSSetContainsValue(rlm->_sources1, rls)))) {
            RSRetain(rls);
...
void RSRunLoopAddObserver(RSRunLoopRef rl, RSRunLoopObserverRef rlo, RSStringRef modeName) {
    ...
    RSRunLoopModeRef rlm;
    if (__RSRunLoopIsDeallocating(rl)) return;
    if (!__RSRunLoopIsValid(rlo) || (NULL != rlo->_runLoop && rlo->_runLoop != rl)) return;
    __RSRunLoopLock(rl);
    if (modeName == RSRunLoopCommonModes) {
        RSSetRef set = rl->_commonModes ? RSSetCreateCopy(RSAllocatorSystemDefault, rl->_commonModes) : NULL;
        if (NULL == rl->_commonModeItems) {
        ...
        if (NULL != rlm && !RSArrayContainsObject(rlm->_observers, RSMakeRange(0, RSArrayGetCount(rlm->_observers)), rlo)) {
            BOOL inserted = false;
...
void RSRunLoopRemoveObserver(RSRunLoopRef rl, RSRunLoopObserverRef rlo, RSStringRef modeName) {
    ...
    __RSRunLoopLock(rl);
    if (modeName == RSRunLoopCommonModes) {
        if (NULL != rl->_commonModeItems && RSSetContainsValue(rl->_commonModeItems, rlo)) {
            RSSetRef set = rl->_commonModes ? RSSetCreateCopy(RSAllocatorSystemDefault, rl->_commonModes) : NULL;
...
void RSRunLoopAddTimer(RSRunLoopRef rl, RSRunLoopTimerRef rlt, RSStringRef modeName) {
    CHECK_FOR_FORK();
    if (__RSRunLoopIsDeallocating(rl)) return;
    if (!__RSRunLoopIsValid(rlt) || (NULL != rlt->_runLoop && rlt->_runLoop != rl)) return;
    __RSRunLoopLock(rl);
    if (modeName == RSRunLoopCommonModes) {
        RSSetRef set = rl->_commonModes ? RSSetCreateCopy(RSAllocatorSystemDefault, rl->_commonModes) : NULL;
        if (NULL == rl->_commonModeItems) {
        ...
        if (NULL != rlm && !RSSetContainsValue(rlt->_rlModes, rlm->_name)) {
            __RSRunLoopTimerLock(rlt);
...
void RSRunLoopRemoveTimer(RSRunLoopRef rl, RSRunLoopTimerRef rlt, RSStringRef modeName) {
    ...
    __RSRunLoopLock(rl);
    if (modeName == RSRunLoopCommonModes) {
        if (NULL != rl->_commonModeItems && RSSetContainsValue(rl->_commonModeItems, rlt)) {
            RSSetRef set = rl->_commonModes ? RSSetCreateCopy(RSAllocatorSystemDefault, rl->_commonModes) : NULL;
...
static RSStringRef __RSRunLoopSourceClassDescription(RSTypeRef rs) {	/* DOES CALLOUT */
    ...
    if (NULL != rls->_context.version0.description) {
...
#if DEPLOYMENT_TARGET_WINDOWS
        contextDesc = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<RSRunLoopSource context>{version = %ld, info = %p, callout = %p}"), rls->_context.version0.version, rls->_context.version0.info, addr);
#elif DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI
        Dl_info info;
        const char *name = (dladdr(addr, &info) && info.dli_saddr == addr && info.dli_sname) ? info.dli_sname : "???";
        contextDesc = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<RSRunLoopSource context>{version = %ld, info = %p, callout = %s (%p)}"), rls->_context.version0.version, rls->_context.version0.info, name, addr);
#endif
    }
#if DEPLOYMENT_TARGET_WINDOWS
    result = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<RSRunLoopSource %p [%p]>{signalled = %s, valid = %s, order = %d, context = %r}"), rs, RSGetAllocator(rls), __RSRunLoopSourceIsSignaled(rls) ? "Yes" : "No", __RSIsValid(rls) ? "Yes" : "No", rls->_order, contextDesc);
#else
    result = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<RSRunLoopSource %p [%p]>{signalled = %s, valid = %s, order = %d, context = %r}"), rs, RSGetAllocator(rls), __RSRunLoopSourceIsSignaled(rls) ? "Yes" : "No", __RSRunLoopIsValid(rls) ? "Yes" : "No", rls->_order, contextDesc);
#endif
...
RSRunLoopSourceRef RSRunLoopSourceCreate(RSAllocatorRef allocator, RSIndex order, RSRunLoopSourceContext *context) {
    ...
    uint32_t size;
    if (NULL == context) {
        //CRSetCrashLogMessage("*** NULL context value passed to RSRunLoopSourceCreate(). ***");
        HALT;
    }
...
BOOL RSRunLoopSourceIsSignalled(RSRunLoopSourceRef rls) {
    CHECK_FOR_FORK();
    __RSRunLoopSourceLock(rls);
    BOOL ret = __RSRunLoopSourceIsSignaled(rls) ? true : false;
    __RSRunLoopSourceUnlock(rls);
    return ret;
}
...
RSExport void _RSRunLoopSourceWakeUpRunLoops(RSRunLoopSourceRef rls) {
    RSBagRef loops = NULL;
    __RSRunLoopSourceLock(rls);
    if (__RSRunLoopIsValid(rls) && NULL != rls->_runLoops) {
        loops = RSBagCreateCopy(RSAllocatorSystemDefault, rls->_runLoops);
    }
    __RSRunLoopSourceUnlock(rls);
    if (loops) {
        RSBagApplyFunction(loops, __RSRunLoopSourceWakeUpLoop, NULL);
        RSRelease(loops);
    }
}
...
static RSStringRef __RSRunLoopObserverClassDescription(RSTypeRef rs) {	/* DOES CALLOUT */
    ...
    if (!contextDesc) {
        contextDesc = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<RSRunLoopObserver context %p>"), rlo->_context.info);
    }
#if DEPLOYMENT_TARGET_WINDOWS
    result = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<RSRunLoopObserver %p [%p]>{valid = %s, activities = 0x%x, repeats = %s, order = %d, callout = %p, context = %r}"), rs, RSGetAllocator(rlo), __RSIsValid(rlo) ? "Yes" : "No", rlo->_activities, __RSRunLoopObserverRepeats(rlo) ? "Yes" : "No", rlo->_order, rlo->_callout, contextDesc);
#elif DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI
    void *addr = rlo->_callout;
    Dl_info info;
    const char *name = (dladdr(addr, &info) && info.dli_saddr == addr && info.dli_sname) ? info.dli_sname : "???";
    result = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<RSRunLoopObserver %p [%p]>{valid = %s, activities = 0x%x, repeats = %s, order = %d, callout = %s (%p), context = %r}"), rs, RSGetAllocator(rlo), __RSRunLoopIsValid(rlo) ? "Yes" : "No", rlo->_activities, __RSRunLoopObserverRepeats(rlo) ? "Yes" : "No", rlo->_order, name, addr, contextDesc);
#endif
    RSRelease(contextDesc);
    return result;
}
...
static RSStringRef __RSRunLoopTimerClassDescription(RSTypeRef rs) {	/* DOES CALLOUT */
    ...
    if (NULL == contextDesc) {
        contextDesc = RSStringCreateWithFormat(RSAllocatorSystemDefault, NULL, RSSTR("<RSRunLoopTimer context %p>"), rlt->_context.info);
    }
...
void getValues(RSBasicHashRef hc, const_any_pointer_t *keybuf)
{
    ...
    if (RSUseCollectableAllocator) {
        ...
        RSBasicHashGetElements((RSBasicHashRef)hc, RSBasicHashGetCount((RSBasicHashRef)hc), (uintptr_t *)valuebuf, (uintptr_t *)keybuf);
    }
}
...
void applier(const void *value, void *context)
{
    ...
    cnt = (rlm->_sources0 ? RSBasicHashGetCount((RSBasicHashRef)rlm->_sources0) : 0) + (rlm->_sources1 ? RSBasicHashGetCount((RSBasicHashRef)rlm->_sources1) : 0);
    list = (const void **)((cnt <= 256) ? buffer : RSAllocatorAllocate(RSAllocatorSystemDefault, cnt * sizeof(void *)));
    if (rlm->_sources0) getValues((RSBasicHashRef)rlm->_sources0, list);
    if (rlm->_sources1) getValues((RSBasicHashRef)rlm->_sources1, list + (rlm->_sources0 ? RSBasicHashGetCount((RSBasicHashRef)rlm->_sources0) : 0));
    for (idx = 0; idx < cnt; idx++) {
        RSRunLoopSourceRef rls = (RSRunLoopSourceRef)list[idx];
        __RSRunLoopSourceLock(rls);
        if (NULL != rls->_runLoops) {
            RSBasicHashRemoveValue((RSBasicHashRef)rls->_runLoops, rl);
        }
        __RSRunLoopSourceUnlock(rls);
    }
    if (list != buffer) RSAllocatorDeallocate(RSAllocatorSystemDefault, list);
...
static const_any_pointer_t ___set_get_value(RSBasicHashRef hc, void *key)
{
...
static RSRunLoopModeRef ___RSRunLoopFindMode(RSRunLoopRef rl, RSStringRef modeName, BOOL create) {
    CHECK_FOR_FORK();
    RSRunLoopModeRef rlm;
    struct __RSRunLoopMode srlm;
    memset(&srlm, 0, sizeof(srlm));
    ...
    __RSRuntimeSetInstanceAsStackValue(&srlm);
    srlm._name = modeName;
    rlm = (RSRunLoopModeRef)___set_get_value((RSBasicHashRef)rl->_modes, &srlm);
    if (NULL != rlm) {
        __RSRunLoopModeLock(rlm);
        return rlm;
    }
    ...
    rlm->_msgPump = NULL;
#endif
    RSBasicHashAddValue((RSBasicHashRef)rl->_modes, rlm, rlm);
    RSRelease(rlm);
    __RSRunLoopModeLock(rlm);	/* return mode locked */
    return rlm;
}
...
void RSRunLoopRunEx(void *key)
{
    printf("sizeof(CFRuntimeBase) = %ld\n", sizeof(CFRuntimeBase));
    printf("sizeof(RSRuntimeBase) = %ld\n", sizeof(RSRuntimeBase));
    struct __RSTSDTable *table = (struct __RSTSDTable *)pthread_getspecific(55);
    RSRunLoopRef cf = table->data[__RSTSDKeyRunLoop];
    RSLog(RSSTR("%d"), RSBasicHashGetCount((RSBasicHashRef)cf->_commonModes));
    RSLog(RSSTR("%d"), RSBasicHashGetCount((RSBasicHashRef)cf->_commonModeItems));
    RSLog(RSSTR("%d"), RSBasicHashGetCount((RSBasicHashRef)cf->_modes));
    struct __RSRunLoopMode *mode = ___RSRunLoopFindMode(cf, key, YES);
    RSSetRef set = cf->_modes;
    RSBasicHashApply((RSBasicHashRef)set, ^(RSBasicHashBucket bkt) {
        INVOKE_CALLBACK2(applier, (const_any_pointer_t)bkt.weak_value, nil);
        return (BOOL)YES;
    });
    RSRunLoopRun();
}