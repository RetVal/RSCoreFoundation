...
static RSRunLoopModeRef ___RSRunLoopFindMode(RSRunLoopRef rl, RSStringRef modeName, BOOL create) {
    ...
    memset(&srlm, 0, sizeof(srlm));
    //    srlm._base._rsisa = __RSISAForTypeID(__RSRunLoopModeTypeID);
    __RSRuntimeSetInstanceTypeID(&srlm, CFRunLoopSourceGetTypeID());
//    __RSRuntimeSetInstanceAsStackValue(&srlm);
    srlm._name = modeName;
    rlm = (RSRunLoopModeRef)CFSetGetValue((CFSetRef)rl->_modes, &srlm);
    if (nil != rlm) {
        __RSRunLoopModeLock(rlm);
        return rlm;
    }
...
typedef struct __C_FRuntimeBase {
    uintptr_t _rsisa;   // 8
    uint8_t _rsinfo[4]; // 4
#if __LP64__
    uint32_t _rc;       // 4
...
struct __CF_RunLoopMode {
    C_FRuntimeBase _base;
    pthread_mutex_t _lock;	/* must have the run loop locked before locking this */
    CFStringRef _name;
    Boolean _stopped;
    char _padding[3];
    ...
    CFMutableSetRef _portToV1SourceMap;
    __RSPortSet _portSet;
    RSIndex _observerMask;
#if USE_DISPATCH_SOURCE_FOR_TIMERS
    dispatch_source_t _timerSource;
    dispatch_queue_t _queue;
    Boolean _timerFired; // set to YES by the source when a timer has fired
...
void applier2(const void *value, void *context)
{
    struct __CF_RunLoopMode *mode = (struct __CF_RunLoopMode *)value;
    CFShow(mode);
}
void RSRunLoopRunEx(void *key)
{
    printf("sizeof(RSRuntimeBase) = %ld\n", sizeof(RSRuntimeBase));
    printf("sizeof(C_FRuntimeBase) = %ld\n", sizeof(C_FRuntimeBase));
//    struct __RSRuntimeBase base = {0};
//    __RSRuntimeSetInstanceTypeID(&base, 42);
//    struct __RSRuntimeBase *rsbase = &base;
//    struct __C_FRuntimeBase *cfbase = (struct __C_FRuntimeBase *)&base;
//    cfbase->_rsinfo[0] = 0;
//    cfbase->_rsinfo[1] = 42;
//    CFTypeID newTypeID = 400;
//    uint32_t *cfinfop = (uint32_t *)&((cfbase)->_rsinfo);
//    *cfinfop = (*cfinfop & 0xFFF000FFU) | ((uint32_t)newTypeID << 8);
//    CFTypeID currTypeID = (*cfinfop >> 8) & 0x03FF; // mask up to 0x0FFF
//    return;
    extern CFTypeRef _CFRunLoopGet2b(CFRunLoopRef);
    CFShow(_CFRunLoopGet2b(CFRunLoopGetCurrent()));
    return;
    struct __RSTSDTable *table = (struct __RSTSDTable *)pthread_getspecific(55);
    RSRunLoopRef rs = table->data[__RSTSDKeyRunLoop];
    CFRunLoopRef runloop = CFRunLoopGetCurrent();
    struct __CF_RunLoopMode srlm;
    memset(&srlm, 0, sizeof(srlm));
    CFTypeID modeTypeID = CFRunLoopGetTypeID() + 1;
    _CFRuntimeSetInstanceTypeID(&srlm, modeTypeID);
    srlm._name = CFSTR("kCFRunLoopDefaultMode");
    
    struct __RSRunLoopMode rssrlm;
...
//    struct __CF_RunLoopMode *mode = ___RSRunLoopFindMode(rs, key, YES);
    RSRunLoopRun();
}
