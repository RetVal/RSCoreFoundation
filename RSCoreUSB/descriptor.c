  ...
  if (usb_interface->altsetting) {
    for (i = 0; i < usb_interface->num_altsetting; i++) {
      struct libusb_interface_descriptor *ifp =
          (struct libusb_interface_descriptor *)usb_interface->altsetting + i;
      if (ifp->extra)
        free((void *)ifp->extra);
      if (ifp->endpoint) {
        for (j = 0; j < ifp->bNumEndpoints; j++)
          clear_endpoint((struct libusb_endpoint_descriptor *)ifp->endpoint +
                         j);
...
static int parse_interface(libusb_context *ctx,
  ...
  while (size >= INTERFACE_DESC_LENGTH) {
    ...
    altsetting =
        realloc(altsetting, sizeof(struct libusb_interface_descriptor) *
    ...
    while (size >= DESC_HEADER_LENGTH) {
      usbi_parse_descriptor(buffer, "bb", &header, 0);
      if (header.bLength < 2) {
        usbi_err(ctx, "invalid descriptor of length %d", header.bLength);
        r = LIBUSB_ERROR_IO;
        goto err;
      }
    ...
    if (size < LIBUSB_DT_INTERFACE_SIZE ||
        ifp->bDescriptorType != LIBUSB_DT_INTERFACE || !ifp->bAlternateSetting)
      return parsed;
  }
  ...
  for (i = 0; i < config->bNumInterfaces; i++) {
    ...
    while (size >= DESC_HEADER_LENGTH) {
      ...
      if ((header.bLength > size) || (header.bLength < DESC_HEADER_LENGTH)) {
        usbi_err(ctx, "invalid descriptor length of %d", header.bLength);
        r = LIBUSB_ERROR_IO;
        goto err;
      }
...
libusb_get_active_config_descriptor(libusb_device *dev,
  ...
  r = usbi_backend->get_active_config_descriptor(
      dev, buf, _config->wTotalLength, &host_endian);
  if (r < 0)
    goto err;
 ...
 * \see libusb_get_config_descriptor()
 */
int API_EXPORTED libusb_get_config_descriptor_by_value(
