...
libusb_control_transfer(libusb_device_handle *dev_handle, uint8_t bmRequestType,
  ...
  libusb_fill_control_transfer(transfer, dev_handle, buffer, ctrl_transfer_cb,
                               &completed, timeout);
  ...
  case LIBUSB_TRANSFER_CANCELLED:
    ...
    break;
  default:
    usbi_warn(HANDLE_CTX(dev_handle), "unrecognised status code %d",
              transfer->status);
...
int API_EXPORTED libusb_bulk_transfer(struct libusb_device_handle *dev_handle,
  ...
  return do_sync_bulk_transfer(dev_handle, endpoint, data, length, transferred,
