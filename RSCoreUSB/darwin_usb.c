...
static int darwin_get_config_descriptor(struct libusb_device *dev,
...
static int ep_to_pipeRef(struct libusb_device_handle *dev_handle, uint8_t ep,
                         uint8_t *pipep, uint8_t *ifcp) {
    struct darwin_device_handle_priv *priv =
    (struct darwin_device_handle_priv *)dev_handle->os_priv;
    ...
    usbi_info(HANDLE_CTX(dev_handle),
    ...
    if (location) {
        ...
        if (propertyMatchDict) {
            ...
            CFDictionarySetValue(matchingDict, CFSTR(kIOPropertyMatchKey),
                                 propertyMatchDict);
            /* release out reference to the CFMutableDictionaryRef
             * (CFDictionarySetValue retains it) */
            CFRelease(propertyMatchDict);
        }
        /* else we can still proceed as long as the caller accounts for the
         * possibility of other devices in the iterator */
    }
...
static usb_device_t **usb_get_next_device(io_iterator_t deviceIterator,
    ...
    while ((usbDevice = IOIteratorNext(deviceIterator))) {
        ...
        usbi_dbg("libusb/darwin.c usb_get_next_device: could not set up plugin for "
                 "service: %s\n",
    ...
    ->QueryInterface(plugInInterface, CFUUIDGetUUIDBytes(DeviceInterfaceID),
                     (LPVOID)&device);
...
static kern_return_t darwin_get_device(uint32_t dev_location,
...
static void darwin_devices_detached(void *ptr, io_iterator_t rem_devices) {
    ...
    while ((device = IOIteratorNext(rem_devices)) != 0) {
        ...
        locationCF = IORegistryEntryCreateCFProperty(
                                                     device, CFSTR(kUSBDevicePropertyLocationID), kCFAllocatorDefault, 0);
        ...
        list_for_each_entry(handle, &ctx->open_devs, list,
            ...
            /* the device may have been opened several times. write to each handle's
             * event descriptor */
...
static void *event_thread_main(void *arg0) {
    ...
    CFRunLoopAddSource(runloop, libusb_notification_cfsource,
    ...
    kresult = IOServiceAddMatchingNotification(
                                               ...
                                               IOServiceMatching(kIOUSBDeviceClassName),
                                               (IOServiceMatchingCallback)darwin_devices_detached, (void *)ctx,
...
static int darwin_get_device_descriptor(struct libusb_device *dev,
...
static int get_configuration_index(struct libusb_device *dev,
    ...
    kresult =
    (*(priv->device))->GetNumberOfConfigurations(priv->device, &numConfig);
    if (kresult != kIOReturnSuccess)
...
static int darwin_get_config_descriptor(struct libusb_device *dev,
    ...
    kresult =
    (*device)->GetConfigurationDescriptorPtr(device, config_index, &desc);
    if (kresult == kIOReturnSuccess) {
        /* copy descriptor */
...
static int darwin_check_configuration(struct libusb_context *ctx,
    ...
    if (priv->dev_descriptor.bNumConfigurations < 1) {
        ...
        return LIBUSB_ERROR_OTHER; /* no configurations at this speed so we can't
                                    use it */
    }
    
    /* find the first configuration */
    kresult = (*darwin_device)
    ->GetConfigurationDescriptorPtr(darwin_device, 0, &configDesc);
    priv->first_config =
    (kIOReturnSuccess == kresult) ? configDesc->bConfigurationValue : 1;
    
    /* check if the device is already configured. there is probably a better way
     than iterating over the
     to accomplish this (the trick is we need to avoid a call to
     GetConfigurations since buggy devices
    ...
    kresult = (*(darwin_device))
    ->CreateInterfaceIterator(darwin_device, &request,
                              &interface_iterator);
    if (kresult)
        return darwin_to_libusb(kresult);
    ...
    if (firstInterface) {
        ...
        if (priv->dev_descriptor.bNumConfigurations == 1)
        /* to avoid problems with some devices get the configurations value from
         * the configuration descriptor */
            priv->active_config = priv->first_config;
        else
        /* devices with more than one configuration should work with
         * GetConfiguration */
    ...
    usbi_info(ctx, "active config: %u, first config: %u", priv->active_config,
              priv->first_config);
    
    return 0;
}

static int darwin_cache_device_descriptor(struct libusb_context *ctx,
                                          struct libusb_device *dev,
                                          usb_device_t **device) {
    ...
    do {
        ...
        /* according to Apple's documentation the device must be open for
         * DeviceRequest but we may not be able to open some
         * devices and Apple's USB Prober doesn't bother to open the device before
         * issuing a descriptor request.  Still,
         * to follow the spec as closely as possible, try opening the device */
        
        ret = (*(device))->DeviceRequest(device, &req);
        
        if (kIOReturnOverrun == ret &&
            kUSBDeviceDesc == priv->dev_descriptor.bDescriptorType)
            ...
            (0 == priv->dev_descriptor.bNumConfigurations ||
             0 == priv->dev_descriptor.bcdUSB)) {
                ...
                if (try_reconfigure && is_open) {
                    usbi_dbg("descriptor appears to be invalid. resetting configuration "
                             "before trying again...");
        ...
        if (kIOReturnSuccess != ret && is_open && try_unsuspend) {
            ...
            /* IOUSBFamily 320+ provides a way to detect device suspension but earlier
             * versions do not */
            ...
            if (try_unsuspend) {
                ...
                if (kIOReturnSuccess != ret2) {
                    ...
                    usbi_dbg(
                             "could not retrieve device descriptor. failed to unsuspend: %s",
                             darwin_error_str(ret2));
                } else
                    unsuspended = 1;
        ...
        if (kIOReturnSuccess != ret) {
            usbi_dbg("kernel responded with code: 0x%08x. sleeping for %d ms before "
                     "trying again",
                     ret, delay / 1000);
    ...
    if (ret != kIOReturnSuccess) {
        ...
        if (LIBUSB_CLASS_HUB == bDeviceClass)
            usbi_dbg(
                     "could not retrieve device descriptor %.4x:%.4x: %s. skipping device",
                     idVendor, idProduct, darwin_error_str(ret));
        else
            usbi_warn(
                      ctx,
                      "could not retrieve device descriptor %.4x:%.4x: %s. skipping device",
    ...
    usbi_dbg(" bNumConfigurations: 0x%02x",
             priv->dev_descriptor.bNumConfigurations);
    
    /* catch buggy hubs (which appear to be virtual). Apple's own USB prober has
...
static int process_new_device(struct libusb_context *ctx, usb_device_t **device,
                              UInt32 locationID,
    ...
    do {
        ...
        /* check current active configuration (and cache the first configuration
         * value-- which may be used by claim_interface) */
        ...
        snprintf(priv->sys_path, 20, "%03i-%04x-%04x-%02x-%02x", address,
                 priv->dev_descriptor.idVendor, priv->dev_descriptor.idProduct,
                 priv->dev_descriptor.bDeviceClass,
                 priv->dev_descriptor.bDeviceSubClass);
...
static int darwin_get_device_list(struct libusb_context *ctx,
                                  struct discovered_devs **_discdevs) {
    io_iterator_t deviceIterator;
    usb_device_t **device;
    kern_return_t kresult;
...
static int darwin_open(struct libusb_device_handle *dev_handle) {
    ...
    (struct darwin_device_handle_priv *)dev_handle->os_priv;
    struct darwin_device_priv *dpriv =
    (struct darwin_device_priv *)dev_handle->dev->os_priv;
    usb_device_t **darwin_device;
    IOReturn kresult;
    ...
    if (0 == dpriv->open_count) {
        ...
        if (kresult != kIOReturnSuccess) {
            ...
            kresult =
            (*(dpriv->device))
            ->CreateDeviceAsyncEventSource(dpriv->device, &priv->cfSource);
            if (kresult != kIOReturnSuccess) {
                usbi_err(HANDLE_CTX(dev_handle), "CreateDeviceAsyncEventSource: %s",
                         darwin_error_str(kresult));
            ...
            CFRunLoopAddSource(libusb_darwin_acfl, priv->cfSource,
...
static int darwin_set_configuration(struct libusb_device_handle *dev_handle,
                                    int config) {
    struct darwin_device_priv *dpriv =
    (struct darwin_device_priv *)dev_handle->dev->os_priv;
    IOReturn kresult;
...
static int darwin_get_interface(usb_device_t **darwin_device, uint8_t ifc,
...
static int get_endpoints(struct libusb_device_handle *dev_handle, int iface) {
    ...
    kresult = (*(cInterface->interface))
    ->GetNumEndpoints(cInterface->interface, &numep);
    if (kresult) {
        usbi_err(HANDLE_CTX(dev_handle),
                 "can't get number of endpoints for interface: %s",
                 darwin_error_str(kresult));
        return darwin_to_libusb(kresult);
    }
    ...
    for (i = 1; i <= numep; i++) {
        ...
        ->GetPipeProperties(cInterface->interface, i, &direction, &number,
        ...
        usbi_info(HANDLE_CTX(dev_handle),
                  "interface: %i pipe %i: dir: %i number: %i", iface, i, direction,
                  number);
        
        cInterface->endpoint_addrs[i - 1] =
        ((direction << 7 & LIBUSB_ENDPOINT_DIR_MASK) |
...
static int darwin_claim_interface(struct libusb_device_handle *dev_handle,
    ...
    if (!usbInterface && dpriv->first_config != 0) {
        usbi_info(HANDLE_CTX(dev_handle),
                  "no interface found; setting configuration: %d",
                  dpriv->first_config);
    ...
    kresult = IOCreatePlugInInterfaceForService(
    ...
    if (kresult) {
        usbi_err(HANDLE_CTX(dev_handle), "IOCreatePlugInInterfaceForService: %s",
                 darwin_error_str(kresult));
        return darwin_to_libusb(kresult);
    }
    ...
    /* create async event source */
    kresult = (*(cInterface->interface))
    ->CreateInterfaceAsyncEventSource(cInterface->interface,
                                      &cInterface->cfSource);
    if (kresult != kIOReturnSuccess) {
...
static int
darwin_set_interface_altsetting(struct libusb_device_handle *dev_handle,
    ...
    kresult = (*(cInterface->interface))
    ->SetAlternateInterface(cInterface->interface, altsetting);
    if (kresult != kIOReturnSuccess)
...
static int darwin_clear_halt(struct libusb_device_handle *dev_handle,
    ...
    kresult = (*(cInterface->interface))
    ->ClearPipeStall(cInterface->interface, pipeRef);
#else
    /* newer versions of darwin support clearing additional bits on the device's
     * endpoint */
    kresult = (*(cInterface->interface))
    ->ClearPipeStallBothEnds(cInterface->interface, pipeRef);
#endif
    if (kresult)
        usbi_err(HANDLE_CTX(dev_handle), "ClearPipeStall: %s",
                 darwin_error_str(kresult));
...
static int darwin_kernel_driver_active(struct libusb_device_handle *dev_handle,
                                       int interface) {
    struct darwin_device_priv *dpriv =
    (struct darwin_device_priv *)dev_handle->dev->os_priv;
    ...
    driver = IORegistryEntryCreateCFProperty(usbInterface, kIOBundleIdentifierKey,
...
/* attaching/detaching kernel drivers is not currently supported (maybe in the
 * future?) */
static int darwin_attach_kernel_driver(struct libusb_device_handle *dev_handle,
                                       int interface) {
    (void)dev_handle;
    (void)interface;
...
static int darwin_detach_kernel_driver(struct libusb_device_handle *dev_handle,
                                       int interface) {
    (void)dev_handle;
    (void)interface;
    return LIBUSB_ERROR_NOT_SUPPORTED;
...
static void darwin_destroy_device(struct libusb_device *dev) { (void)dev; }

static int submit_bulk_transfer(struct usbi_transfer *itransfer) {
    struct libusb_transfer *transfer =
    USBI_TRANSFER_TO_LIBUSB_TRANSFER(itransfer);
    struct darwin_device_handle_priv *priv =
    (struct darwin_device_handle_priv *)transfer->dev_handle->os_priv;
    ...
    (*(cInterface->interface))
    ->GetPipeProperties(cInterface->interface, pipeRef, &direction, &number,
    ...
    if (transferType == kUSBInterrupt) {
        if (IS_XFERIN(transfer))
            ret = (*(cInterface->interface))
            ->ReadPipeAsync(cInterface->interface, pipeRef,
        ...
        if (IS_XFERIN(transfer))
            ret = (*(cInterface->interface))
            ->ReadPipeAsyncTO(cInterface->interface, pipeRef,
                              transfer->buffer, transfer->length,
                              transfer->timeout, transfer->timeout,
                              darwin_async_io_callback, (void *)itransfer);
        else
            ret = (*(cInterface->interface))
            ->WritePipeAsyncTO(cInterface->interface, pipeRef,
                               transfer->buffer, transfer->length,
                               transfer->timeout, transfer->timeout,
                               darwin_async_io_callback, (void *)itransfer);
    }
    ...
    if (ret)
        usbi_err(TRANSFER_CTX(transfer),
                 "bulk transfer failed (dir = %s): %s (code = 0x%08x)",
                 IS_XFERIN(transfer) ? "In" : "Out", darwin_error_str(ret), ret);
...
static int submit_iso_transfer(struct usbi_transfer *itransfer) {
    ...
    if (!tpriv->isoc_framelist) {
        tpriv->num_iso_packets = transfer->num_iso_packets;
        tpriv->isoc_framelist = (IOUSBIsocFrame *)calloc(transfer->num_iso_packets,
                                                         sizeof(IOUSBIsocFrame));
        if (!tpriv->isoc_framelist)
            return LIBUSB_ERROR_NO_MEM;
    }
    
    /* copy the frame list from the libusb descriptor (the structures differ only
     * is member order) */
    ...
    kresult = (*(cInterface->interface))
    ->GetBusFrameNumber(cInterface->interface, &frame, &atTime);
    if (kresult) {
    ...
    if (cInterface->frames[transfer->endpoint] &&
    ...
    frame + transfer->num_iso_packets / 8;
    
    if (kresult != kIOReturnSuccess) {
        usbi_err(TRANSFER_CTX(transfer),
                 "isochronous transfer failed (dir: %s): %s",
                 IS_XFERIN(transfer) ? "In" : "Out", darwin_error_str(kresult));
        free(tpriv->isoc_framelist);
        tpriv->isoc_framelist = NULL;
    }
...
static int submit_control_transfer(struct usbi_transfer *itransfer) {
    ...
    struct darwin_device_priv *dpriv =
    (struct darwin_device_priv *)transfer->dev_handle->dev->os_priv;
    struct darwin_device_handle_priv *priv =
    (struct darwin_device_handle_priv *)transfer->dev_handle->os_priv;
    struct darwin_transfer_priv *tpriv = usbi_transfer_get_os_priv(itransfer);
    ...
    if (transfer->endpoint) {
        ...
        kresult = (*(cInterface->interface))
        ->ControlRequestAsyncTO(cInterface->interface, pipeRef,
                                &(tpriv->req),
                                darwin_async_io_callback, itransfer);
    } else
    /* control request on endpoint 0 */
        kresult = (*(dpriv->device))
        ->DeviceRequestAsyncTO(dpriv->device, &(tpriv->req),
                               darwin_async_io_callback, itransfer);
...
static void darwin_clear_transfer_priv(struct usbi_transfer *itransfer) {
    ...
    if (transfer->type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS &&
        tpriv->isoc_framelist) {
        free(tpriv->isoc_framelist);
        tpriv->isoc_framelist = NULL;
    }
}

static void darwin_async_io_callback(void *refcon, IOReturn result,
                                     void *arg0) {
...
static int darwin_transfer_status(struct usbi_transfer *itransfer,
    ...
    switch (result) {
        ...
        default:
            usbi_err(ITRANSFER_CTX(itransfer), "transfer error: %s (value = 0x%08x)",
                     darwin_error_str(result), result);
...
static void darwin_handle_callback(struct usbi_transfer *itransfer,
    ...
    usbi_info(
              ITRANSFER_CTX(itransfer), "handling %s completion with kernel status %d",
              isControl ? "control" : isBulk ? "bulk" : isIsoc ? "isoc" : "interrupt",
              result);
    ...
    usbi_handle_transfer_completion(itransfer,
                                    darwin_transfer_status(itransfer, result));
}
