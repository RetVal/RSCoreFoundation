...
const struct libusb_version libusb_version_internal = {
    LIBUSB_MAJOR, LIBUSB_MINOR, LIBUSB_MICRO,
    LIBUSB_NANO,  LIBUSB_RC,    LIBUSB_DESCRIBE};
...
struct discovered_devs *discovered_devs_append(struct discovered_devs *discdevs,
  ...
  discdevs = realloc(discdevs, sizeof(*discdevs) + (sizeof(void *) * capacity));
  if (discdevs) {
...
struct libusb_device *usbi_get_device_by_session_id(struct libusb_context *ctx,
  ...
  list_for_each_entry(dev, &ctx->usb_devs, list,
                      struct libusb_device) if (dev->session_data ==
                                                session_id) {
    ret = dev;
    break;
  }
...
find_endpoint(struct libusb_config_descriptor *config, unsigned char endpoint) {
  int iface_idx;
...
libusb_get_max_packet_size(libusb_device *dev, unsigned char endpoint) {
  ...
  if (r < 0) {
    usbi_err(DEVICE_CTX(dev), "could not retrieve active config descriptor");
...
static void do_close(struct libusb_context *ctx,
  ...
  list_for_each_entry_safe(itransfer, tmp, &ctx->flying_transfers, list,
    ...
    if (!(itransfer->flags & USBI_TRANSFER_DEVICE_DISAPPEARED)) {
      usbi_err(ctx, "Device handle closed while transfer was still being "
                    "processed, but the device is still connected as far as we "
                    "know");

      if (itransfer->flags & USBI_TRANSFER_CANCELLING)
        usbi_warn(ctx, "A cancellation for an in-flight transfer hasn't "
                       "completed but closing the device handle");
      else
        usbi_err(ctx, "A cancellation hasn't even been scheduled on the "
                      "transfer for which the device is closing");
    }
     ...
     */
    usbi_dbg("Removed transfer %p from the in-flight list because device "
             "handle %p closed",
             transfer, dev_handle);
  }
...
libusb_get_configuration(libusb_device_handle *dev, int *config) {
  ...
  if (r == LIBUSB_ERROR_NOT_SUPPORTED) {
    ...
    r = libusb_control_transfer(dev, LIBUSB_ENDPOINT_IN,
                                LIBUSB_REQUEST_GET_CONFIGURATION, 0, 0, &tmp, 1,
                                1000);
    if (r == 0) {
 ...
 */
int API_EXPORTED
libusb_kernel_driver_active(libusb_device_handle *dev, int interface_number) {
  usbi_dbg("interface %d", interface_number);
...
int API_EXPORTED libusb_init(libusb_context **context) {
  ...
  usbi_dbg("libusb-%d.%d.%d%s%s%s", libusb_version_internal.major,
...
void usbi_log_v(struct libusb_context *ctx, enum usbi_log_level level,
  ...
  fprintf(stream, "libusb: %d.%06d %s [%s] ", (int)now.tv_sec, (int)now.tv_usec,
          prefix, function);
