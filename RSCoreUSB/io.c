...
int actual_length;
int r = libusb_bulk_transfer(handle, LIBUSB_ENDPOINT_IN, data, sizeof(data),
&actual_length, 0);
...
while (user has not requested application exit) {
        ...
        poll(on libusb file descriptors plus any other event sources of
interest,
...
0);
        libusb_fill_control_transfer(transfer, dev, buffer, cb, &completed,
1000);
...
if (libusb_try_lock_events(ctx) == 0) {
        ...
        while (!completed) {
                // now that we have the event waiters lock, double check that
another
                // thread is still handling events for us. (it may have ceased
handling
                // events in the time it took us to reach this point)
                if (!libusb_event_handler_active(ctx)) {
                        // whoever was handling events is no longer doing so,
try again
...
#ifdef USBI_TIMERFD_AVAILABLE
  ctx->timerfd =
      timerfd_create(usbi_backend->get_timerfd_clockid(), TFD_NONBLOCK);
  if (ctx->timerfd >= 0) {
...
static int calculate_timeout(struct usbi_transfer *transfer) {
  ...
  unsigned int timeout = USBI_TRANSFER_TO_LIBUSB_TRANSFER(transfer)->timeout;
...
struct libusb_transfer *LIBUSB_CALL libusb_alloc_transfer(int iso_packets) {
  ...
  size_t alloc_size =
      sizeof(struct usbi_transfer) + sizeof(struct libusb_transfer) +
...
int API_EXPORTED libusb_submit_transfer(struct libusb_transfer *transfer) {
  ...
  if (r) {
    ...
    const struct itimerspec it = {
        {0, 0}, {itransfer->timeout.tv_sec, itransfer->timeout.tv_usec * 1000}};
...
static int arm_timerfd_for_next_timeout(struct libusb_context *ctx) {
  ...
  list_for_each_entry(transfer, &ctx->flying_transfers, list,
    ...
    if (!(transfer->flags & USBI_TRANSFER_TIMED_OUT)) {
      ...
      usbi_dbg("next timeout originally %dms",
...
libusb_wait_for_event(libusb_context *ctx, struct timeval *tv) {
  ...
  r = usbi_cond_timedwait(&ctx->event_waiters_cond, &ctx->event_waiters_lock,
                          &timeout);
  return (r == ETIMEDOUT);
}
...
static void handle_timeout(struct usbi_transfer *itransfer) {
  ...
  if (r < 0)
    usbi_warn(TRANSFER_CTX(transfer), "async cancel failed %d errno=%d", r,
              errno);
}
...
static int handle_timeouts_locked(struct libusb_context *ctx) {
  ...
  list_for_each_entry(transfer, &ctx->flying_transfers, list,
    ...
    if (transfer->flags &
        (USBI_TRANSFER_TIMED_OUT | USBI_TRANSFER_OS_HANDLES_TIMEOUT))
      continue;
...
void usbi_remove_pollfd(struct libusb_context *ctx, int fd) {
  ...
  list_for_each_entry(ipollfd, &ctx->pollfds, list,
                      struct usbi_pollfd) if (ipollfd->pollfd.fd == fd) {
    found = 1;
    break;
  }
...
void usbi_handle_disconnect(struct libusb_device_handle *handle) {
  ...
  usbi_dbg("device %d.%d", handle->dev->bus_number,
  ...
  while (1) {
    ...
    to_cancel = NULL;
    list_for_each_entry(
        cur, &HANDLE_CTX(handle)->flying_transfers, list,
        struct usbi_transfer) if (USBI_TRANSFER_TO_LIBUSB_TRANSFER(cur)
                                      ->dev_handle == handle) {
      to_cancel = cur;
      break;
    }
