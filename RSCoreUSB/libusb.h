...
static inline void libusb_fill_control_transfer(
  ...
  if (setup)
    transfer->length =
        LIBUSB_CONTROL_SETUP_SIZE + libusb_le16_to_cpu(setup->wLength);
...
static inline void libusb_fill_iso_transfer(
    struct libusb_transfer *transfer, libusb_device_handle *dev_handle,
    unsigned char endpoint, unsigned char *buffer, int length,
    int num_iso_packets, libusb_transfer_cb_fn callback, void *user_data,
    unsigned int timeout) {
  transfer->dev_handle = dev_handle;
  transfer->endpoint = endpoint;
 ...
 */
static inline void
libusb_set_iso_packet_lengths(struct libusb_transfer *transfer,
                              unsigned int length) {
  int i;
 ...
 */
static inline unsigned char *
libusb_get_iso_packet_buffer(struct libusb_transfer *transfer,
                             unsigned int packet) {
  int i;
  size_t offset = 0;
  int _packet;
 ...
 */
static inline unsigned char *
libusb_get_iso_packet_buffer_simple(struct libusb_transfer *transfer,
                                    unsigned int packet) {
  int _packet;
...
static inline int libusb_get_descriptor(libusb_device_handle *dev,
  ...
  return libusb_control_transfer(
      ...
      (desc_type << 8) | desc_index, 0, data, (uint16_t)length, 1000);
}
...
libusb_get_string_descriptor(libusb_device_handle *dev, uint8_t desc_index,
  ...
  return libusb_control_transfer(
      dev, LIBUSB_ENDPOINT_IN, LIBUSB_REQUEST_GET_DESCRIPTOR,
      (uint16_t)((LIBUSB_DT_STRING << 8) | desc_index), langid, data,
